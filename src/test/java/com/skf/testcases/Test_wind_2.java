...
public class Test_wind_2 extends Page {
  ...
  @Test
  public void verifyEventAndWorkOrder() throws Exception {
	  
	  fisco = new FileInputStream(path + "\\src\\test\\resources\\properties\\Config.properties");
		config.load(fisco);
		LoginPage loginPage = new LoginPage();
		loginPage.loginApp(config.getProperty("test_wind_2_login"), config.getProperty("test_wind_2_pass"));
		EventWorkAndOrderPage eventWorkAndOrderPage = new EventWorkAndOrderPage();
		ElementExisitsPage elemnetExixitsPage = new ElementExisitsPage();
		elemnetExixitsPage.isTurbineOverviewPresent();
